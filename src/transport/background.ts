...
import type { RequestMessage } from '../message-tool/extenison-port-message'
import { PortMessage } from '../message-tool/extenison-port-message'
...
export function init_bg_transport(): void {
  ...
  const onceSessionEndCbs = new Map<UID, Set<() => void>>()
  ...
  const notifyEndpoint = (endpoint: string) => ({
    withFingerprint: (fingerprint: UID) => {
      ...
      const notifications = {
        ...
        aboutSessionEnded: (endedSessionFingerprint: UID) => {
  ...
  const sessFingerprint = createUID()
