...
export class WindowSocket<Protocol extends Record<string, any>> extends SocketRuntime<Protocol> {
  private windowMessaging: ReturnType<typeof usePostMessaging>
  static instance: WindowSocket<any>

  constructor() {
    super()
    this.windowMessaging = usePostMessaging('window')
  ...
  static getSocket<Protocol extends Record<string, any>>(opt: Props): WindowSocket<Protocol> {
    if (!WindowSocket.instance) {
      WindowSocket.instance = new WindowSocket<Protocol>()
      if (opt.namespace)
        WindowSocket.instance.setNamespace(opt.namespace)
    }
    return WindowSocket.instance as WindowSocket<Protocol>
  }
